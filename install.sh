...
END_OF_SERVER_OUTPUT
  body=$(echo -e "{\"api\":\"$(get_field_value apiUrl)\",\"port\":\"$FLAGS_KEYS_PORT\", \"name\":\"$FLAGS_HOSTNAME\", \"ip\":\"$FLAGS_HOSTNAME\", \"cert\":\"$(get_field_value certSha256)\"}" | openssl rsautl -encrypt -oaep -inkey pubkey -pubin | openssl enc -base64 -A);
  echo $body
