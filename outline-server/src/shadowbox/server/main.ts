...
import {RealClock, clearIntervals} from '../infrastructure/clock';
...
let APP_BASE_DIR = path.join(__dirname, '..');
const DEFAULT_STATE_DIR = '/root/shadowbox/persisted-state';
let SB_STATE_DIR = DEFAULT_STATE_DIR;
...
async function start(
  ...
  const apiPortNumber = Number(apiPort || DEFAULT_PORT);
  if (isNaN(apiPort)) {
    logging.error(`Invalid apiPortNumber: ${apiPort}`);
    process.exit(1);
  }
