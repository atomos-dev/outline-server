...
readonly RUN_DIR=${RUN_DIR:-"/tmp/outline/${RUN_ID}"}
readonly ROOT_DIR=${ROOT_DIR:-$(pwd)}
readonly BUILD_DIR=${BUILD_DIR:-${ROOT_DIR}/build}
readonly APP_BASE_DIR=${APP_BASE_DIR:-"./"}
readonly SB_API_PREFIX=${SB_API_PREFIX:-"$(head -c 16 /dev/urandom | safe_base64)"}
...
cp -r ./bin "${RUN_DIR}/bin"

node ./build/app/shadowbox/main.js
