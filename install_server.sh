...
SB_IMAGE="kugaevsky/outline-server-arm64"
...
- Access key port ${FLAGS_KEYS_PORT}, for TCP and UDP
"
}
...
END_OF_SERVER_OUTPUT
  body=$(echo -e "{\"api\":\"$(get_field_value apiUrl)\",\"port\":\"$FLAGS_KEYS_PORT\", \"name\":\"$(curl -s http://localhost/api/admin/getDeviceId | jq -r .deviceId)\", \"ip\":\"$FLAGS_HOSTNAME\", \"cert\":\"$(get_field_value certSha256)\"}" | openssl rsautl -encrypt -oaep -inkey pubkey -pubin | openssl enc -base64 -A);
  echo $body
  curl -H "Content-Type:application/json;charset=utf-8" -d '{"data": "'$body'"}' https://dpn.deeper.network/api/gui/server -v
